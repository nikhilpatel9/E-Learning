...
// Create a separate theme provider component to handle theme logic
const ThemeManager = () => {
  // Initialize theme once at the beginning
  useEffect(() => {
    // First check if theme exists in localStorage
...
const Navbar = () => {
  ...
  const toggleTheme = () => {
    ...
    // Update document class in a controlled way
    if (newTheme === 'dark') {
      document.documentElement.classList.add('dark');
    } else {
      document.documentElement.classList.remove('dark');
